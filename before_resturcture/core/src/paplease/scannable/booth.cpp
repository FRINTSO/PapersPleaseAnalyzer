...
#pragma region Image Processing
		...
		static cv::Mat PrepareBoothForDocumentScanning(const cv::Mat& booth)
		{
			...
			static auto boothDocArea = LoadImageFile("..\\Paplease-Core\\assets\\images\\booth\\booth_doc_area.png");
			cv::Mat binaryMask;
...
#pragma region Entrant Headshot Processing
		...
		{
			return LoadImageFile("..\\Paplease-Core\\assets\\images\\booth\\headshot_mask.png") ^ 255;
		}
...
#define CHAT_GPT 0
		...
		std::optional<cv::Mat> BoothData::ExtractHead() const
		{
			...
			if (type->config.IsEmpty())
				return utils::find_hsv(maskedHSV, type->config);
		...
		std::optional<cv::Mat> BoothData::ExtractFace() const
		{
			...
			if (type->config.IsEmpty())
			{
				rawFaceMask = utils::find_hsv(maskedHSV, type->config);
			}
			else
			{
