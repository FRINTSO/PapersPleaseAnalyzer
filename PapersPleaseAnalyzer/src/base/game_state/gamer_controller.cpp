...
#pragma region Mediator
		...
		const documents::v2::Data& GameAnalysisController::RequestData(const documents::v2::DataType dataType) const
		{
			switch (dataType)
			{
				case paplease::documents::v2::DataType::Date:
				{
		...
		const documents::data::Date& GameAnalysisController::RequestCurrentDate() const
		{
			return m_gameState.GetCurrentDate();
		}
