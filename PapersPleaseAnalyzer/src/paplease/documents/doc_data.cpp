...
#pragma region Constructors
        ...
        Data::Data(data::Date&& data)
            : Data{ std::move(data), DataType::Date }
        {}
...
#pragma region DocData
        ...
        core::OptRef<const Field> DocData::GetField(FieldCategory category, bool returnBroken) const
        {
            ...
            if (!m_fieldLookup.Contains(category))
                return std::nullopt;

            index_type index = m_fieldLookup.Get(category);
            const auto& field = m_fieldStorage[index];
        ...
        void DocData::AddField(Field&& field)
        {
            ...
            assert(category != FieldCategory::Invalid && category != FieldCategory::Count && "Cannot add an invalid field to DocData.");
            assert(!m_fieldStorage.Full() && "Field storage is full!");

            m_fieldStorage.Add(std::move(field));
            index_type index = m_fieldStorage.Count() - 1;

            m_fieldLookup.Set(category, std::forward<index_type>(index));
        }

        bool DocData::TryAddField(Field&& field)
        {
            FieldCategory category = field.Category();
            assert(category != FieldCategory::Invalid && category != FieldCategory::Count && "Cannot add an invalid field to DocData.");
...
#pragma region DocDataBuilder
        ...
        bool DocDataBuilder::AddField(Field&& data, FieldCategory category)
        {
            ...
            return m_data.TryAddField(std::move(data));
        }
        
        DocData DocDataBuilder::Build()
        {
            for (auto& field : m_data.m_fieldStorage)
            {
                field.m_data = utils::ProcessFieldData(field.m_data, field.m_fieldType, field.m_fieldCategory);
                field.m_fieldState = field.IsBroken() ? FieldState::BrokenData : FieldState::ProcessedData;
            }

            DocData builtData = std::move(m_data);
            m_data.Reset();
            return builtData;
        }
