...
static const charset_table &charset_for(typeface tf)
{
	switch (tf) {
	case typeface::t04b03:     return charset_t04b03;
	case typeface::bm_mini:    return charset_bm_mini;
	case typeface::mini_kylie: return charset_mini_kylie;
	case typeface::booth:      return charset_booth;
	}
	unreachable();
}

static char charset_lookup(const charset_table &table, u64 encoding)
{
	for (u32 i = 0; i < table.count; i++)
		if (table.entries[i].encoding == encoding)
			return table.entries[i].ch;
	return '\0';
}
					       ...
					       const font_info &font)
{
	...
	while (line_top != -1) {
		int line_bottom = std::min(line_top + font.max_pixels_tall, image.rows);
		scan_line_for_boxes(boxes, image, line_top, line_bottom, font);
		line_top = find_next_top_line(image, line_top, font.max_pixels_tall);
	}
...
bool extract_text_strict(std::string &out, const cv::Mat &binary_image, typeface tf)
{
	...
	std::vector<rectangle> boxes = extract_character_boxes(binary_image, font);

	const charset_table &table = charset_for(tf);
	if (table.count == 0) {
	...
	for (const auto &box : boxes) {
		...
		char ch = charset_lookup(table, encoded_char);
		if (encoded_char == 0 || ch == '\0') {
		...
		if (previous_rectangle) { // add spaces
			int horizontal_space_from_last_box = box.x - (previous_rectangle->x + previous_rectangle->width);
			const int letter_space_size = font.letter_spacing_horizontal;
			const int whitespace_size = font.whitespace_pixel_width;
		...
		if (previous_rectangle) { // add newlines
			int vertical_space_from_last_box = box.y - (previous_rectangle->y + previous_rectangle->height);
			...
			if (vertical_space_from_last_box - wrapSize >= 0) { // add extra space when wrapping
				field.push_back(' ');
			}
