...
static bool is_border_color_at(const cv::Mat& image, int x, int y, const doc_appearance& app)
{
...
rectangle find_doc_bounding_box(const cv::Mat& image, const doc_appearance& app)
{
    ...
    for (int y = 0; y < max_y; y++) {
        ...
        for (int x = min_x; x < max_x; x++) {
            ...
            if (!found_first) {
                ...
                if (br_x < image.cols && br_y < image.rows) {
                    if (is_border_color_at(image, br_x, br_y, app))
                        return { x, y, (int)app.width, (int)app.height };
                }
            }
        ...
        if (row_matched) {
            if (y < top) {
                top = y;
                max_y = std::min(image.rows, top + (int)app.height);
            }
            bottom = std::max(bottom, y);
            
            if (row_min_x < left) {
                left = row_min_x;
                max_x = std::min(image.cols, left + (int)app.width);
            }
            if (row_max_x > right) {
                right = row_max_x;
...
static bool is_valid_corner(const cv::Mat& image, const rectangle& bbox, 
                            bool is_top, bool is_left, const doc_appearance& app)
{
    ...
    while (step < bbox.width && step < bbox.height) {
        if (!is_border_color_at(image, x_corner + step * dx, y_corner + step * dy, app))
            break;
        step++;
    }
...
bool validate_doc_bounding_box(const cv::Mat& image, const rectangle& bbox, const doc_appearance& app)
{
    // Reject if too large
    ...
    if (is_valid_corner(image, bbox, true, true, app))   return true;  // top-left
    if (is_valid_corner(image, bbox, true, false, app))  return true;  // top-right
    if (is_valid_corner(image, bbox, false, true, app))  return true;  // bottom-left
    if (is_valid_corner(image, bbox, false, false, app)) return true;  // bottom-right
